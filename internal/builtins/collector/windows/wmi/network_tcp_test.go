...
func TestNewNetTCPCollector(t *testing.T) {
	...
	{
		c, err := NewNetTCPCollector(filepath.Join("testdata", "config_metrics_enabled_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if len(c.(*NetTCP).metricStatus) == 0 {
			t.Fatalf("expected >0 metric status settings, got (%#v)", c.(*NetTCP).metricStatus)
		}
		enabled, ok := c.(*NetTCP).metricStatus["foo"]
		if !ok {
			t.Fatalf("expected 'foo' key in metric status settings, got (%#v)", c.(*NetTCP).metricStatus)
		}
		if !enabled {
			t.Fatalf("expected 'foo' to be enabled in metric status settings, got (%#v)", c.(*NetTCP).metricStatus)
		}
	}

	t.Log("config (metrics disabled setting)")
	{
		c, err := NewNetTCPCollector(filepath.Join("testdata", "config_metrics_disabled_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if len(c.(*NetTCP).metricStatus) == 0 {
		...
		if enabled {
			t.Fatalf("expected 'foo' to be disabled in metric status settings, got (%#v)", c.(*NetTCP).metricStatus)
		}
	}

	t.Log("config (metrics default status enabled)")
	{
		c, err := NewNetTCPCollector(filepath.Join("testdata", "config_metrics_default_status_enabled_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if !c.(*NetTCP).metricDefaultActive {
			t.Fatal("expected true")
		}
	}
	...
	{
		c, err := NewNetTCPCollector(filepath.Join("testdata", "config_metrics_default_status_disabled_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if c.(*NetTCP).metricDefaultActive {
	...
	{
		_, err := NewNetTCPCollector(filepath.Join("testdata", "config_metrics_default_status_invalid_setting"))
		if err == nil {
			t.Fatal("expected error")
		}
	}
