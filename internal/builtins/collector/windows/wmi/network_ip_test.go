...
func TestNewNetIPCollector(t *testing.T) {
	...
	{
		c, err := NewNetIPCollector(filepath.Join("testdata", "config_metrics_enabled_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if len(c.(*NetIP).metricStatus) == 0 {
			t.Fatalf("expected >0 metric status settings, got (%#v)", c.(*NetIP).metricStatus)
		}
		enabled, ok := c.(*NetIP).metricStatus["foo"]
		if !ok {
			t.Fatalf("expected 'foo' key in metric status settings, got (%#v)", c.(*NetIP).metricStatus)
		}
		if !enabled {
			t.Fatalf("expected 'foo' to be enabled in metric status settings, got (%#v)", c.(*NetIP).metricStatus)
		}
	}

	t.Log("config (metrics disabled setting)")
	{
		c, err := NewNetIPCollector(filepath.Join("testdata", "config_metrics_disabled_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if len(c.(*NetIP).metricStatus) == 0 {
		...
		if enabled {
			t.Fatalf("expected 'foo' to be disabled in metric status settings, got (%#v)", c.(*NetIP).metricStatus)
		}
	}

	t.Log("config (metrics default status enabled)")
	{
		c, err := NewNetIPCollector(filepath.Join("testdata", "config_metrics_default_status_enabled_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if !c.(*NetIP).metricDefaultActive {
			t.Fatal("expected true")
		}
	}
	...
	{
		c, err := NewNetIPCollector(filepath.Join("testdata", "config_metrics_default_status_disabled_setting"))
		if err != nil {
			t.Fatalf("expected NO error, got (%s)", err)
		}
		if c.(*NetIP).metricDefaultActive {
	...
	{
		_, err := NewNetIPCollector(filepath.Join("testdata", "config_metrics_default_status_invalid_setting"))
		if err == nil {
			t.Fatal("expected error")
		}
	}
