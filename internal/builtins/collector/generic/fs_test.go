...
func TestNewFSCollector(t *testing.T) {
	...
	tests := []struct {
		...
		{"no config", "", "builtins.generic.fs config: invalid config file (empty)", true},
		{"missing config", filepath.Join("testdata", "missing"), "", false},
		{"bad syntax", filepath.Join("testdata", "bad_syntax"), "builtins.generic.fs config: parsing configuration file (testdata/bad_syntax.json): invalid character ',' looking for beginning of value", true},
