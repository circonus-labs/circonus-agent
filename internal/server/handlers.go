...
func (s *Server) run(w http.ResponseWriter, r *http.Request) {
	...
	for cm := range conduitCh {
		// s.logger.Debug().Str("conduit_id", cm.id).Int("numMetrics", len(*cm.metrics)).Msg("adding metrics")
		for m, v := range *cm.metrics {
			metrics[m] = v
		}
	}
	s.logger.Debug().Int("numMetrics", len(metrics)).Msg("aggregated")
...
func (s *Server) encodeResponse(m *cgm.Metrics, w http.ResponseWriter, r *http.Request, runStart time.Time) {
	...
	s.logger.Info().Str("duration", time.Since(runStart).String()).Int("numMetrics", len(*m)).Bool("compressed", useGzip).Int("content_bytes", len(data)).Msg("request response")
